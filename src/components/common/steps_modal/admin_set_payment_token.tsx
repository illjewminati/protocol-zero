...
import { initBalances, setPaymasterMinBalance, setPaymasterPaymentToken } from '../../../store/actions';
import { StepSetPaymentToken } from '../../../types/steps-modal';
...
class SetPaymentTokenStep extends React.Component<Props> {
    public render = () => {
        const { buildStepsProgress, step } = this.props;

        const title = `Set Payment Token`;
...
const mapStateToProps = (state: StoreState): StateProps => {
    return {
        step: getStepsModalCurrentStep(state) as StepSetPaymentToken,
    };
};

const mapDispatchToProps = (dispatch: any): DispatchProps => {
    return {
        onSend: (address: string) => 
            dispatch(setPaymasterPaymentToken(address)),
        initBalances: () => dispatch(initBalances())
    }
}

const SetPaymentTokenStepContainer = connect(
    mapStateToProps,
    mapDispatchToProps
)(SetPaymentTokenStep);

export { SetPaymentTokenStep, SetPaymentTokenStepContainer };
